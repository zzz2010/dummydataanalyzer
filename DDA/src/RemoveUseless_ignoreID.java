 ...
 */
public class RemoveUseless_ignoreID 
  extends Filter 
  ...
  public boolean batchFinished() throws Exception {
    ...
    if (m_removeFilter == null) {
      ...
      int numToDelete = 0;
      for(int i = 1; i < toFilter.numAttributes(); i++) {
	...
	if (stats.distinctCount < 2) {
	  ...
	  double dominatePercent=(double)max(stats.nominalCounts)/(double)(stats.totalCount - stats.missingCount) * 100.0;
	  if(dominatePercent>m_maxVariancePercentage)
	  {
		  attsToDelete[numToDelete++] = i;
	  }
  ...
  public static void main(String [] argv) {
    runFilter(new RemoveUseless_ignoreID(), argv);
  }
}
